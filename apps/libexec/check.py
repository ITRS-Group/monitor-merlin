...
def check_min_avg_max(args, col, defaults=False, filter=False):
    ...
    print (
        "%s: %s %s min/avg/max = %.2f/%.2f/%.2f|%s"
        % (
            nplug.state_name(state),
            otype,
            col,
            values["min"],
            values["avg"],
...
def cmd_exectime(args=False):
    ...
    """
    thresh = {"warning": [5, 30, 65], "critical": [15, 45, 90]}
...
def cmd_latency(args=False):
    ...
    """
    thresh = {"warning": [30, 60, 90], "critical": [60, 90, 120]}
