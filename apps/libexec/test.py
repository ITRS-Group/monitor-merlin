...
class fake_peer_group(object):
    ...
    def __init__(self, basepath, group_name, num_nodes=3, port=0, **kwargs):
        ...
        print (
            "Created peer group '%s' with %d nodes and base-port %d"
    ...
    def create_object_config(self, num_hosts, num_services_per_host):
        ...
        print (
            "Generating object config for %s group %s"
            % (self.group_type, self.group_name)
        )
        if len(self.master_groups):
        ...
        while i < num_hosts:
            ...
            if num_hosts > 50 and os.isatty(sys.stdout.fileno()) and not i % 7:
                sys.stdout.write(
                    "\r%d hosts and %d services created"
                    % (i, i * num_services_per_host)
                )
            ...
            while x < num_services_per_host:
                ...
                sobj["service_description"] = sdesc.replace(
                    "@@service_num@@", "%04d" % x
                )
        ...
        if num_hosts > 50:
            print (
                "\r%d hosts and %d services created" % (i, i * num_services_per_host)
            )
        self.oconf_buf = "\n".join(ocbuf)
        ocbuf = False
        poller_oconf_buf = ""
        ...
        print (
            "  hosts=%d; services=%d; hostgroups=%d; servicegroups=%d"
            % (
                self.num_objects["host"],
                self.num_objects["service"],
                self.num_objects["hostgroup"],
                self.num_objects["servicegroup"],
            )
        )
        return True
...
class fake_instance(object):
        ...
        for name, program in list(daemons.items()):
            ...
            if self.valgrind:
                ...
                real_cmd = [
                    ...
                    "--leak-check=full",
                    "--log-file=%s/valgrind.%s.%d" % (self.home, name, log_id),
                ] + cmd
                cmd = real_cmd
            try:
    ...
    def create_core_config(self):
        ...
        if not len(self.group.master_groups):
            configs[
                "%s/etc/oconf/shared.cfg" % (self.home)
    ...
    def db_connect(self):
        ...
        self.mconf = mconf_mockup(
            host="localhost", user="merlin", dbname=self.db_name, dbpass="merlin"
        )
...
class fake_mesh(object):
    ...
    def intermission(self, msg, sleeptime=False):
        ...
        if os.isatty(sys.stdout.fileno()) == False:
            ...
            while i > 0:
                ...
                if msg:
                    sys.stdout.write(
                        " %c Sleeping %.2f seconds: %s    \r"
                        % (rotor[r % len(rotor)], i, msg)
                    )
                    sys.stdout.flush()
                else:
                    sys.stdout.write(
                        " %c Sleeping %.2f seconds   \r" % (rotor[r % len(rotor)], i)
                    )
                    sys.stdout.flush()
                i -= 0.07
                time.sleep(0.07)
    ...
    def _test_proc_alive(self, proc, exp, sig_ok, msg_prefix, sub):
        ...
        if os.WIFSIGNALED(status):
            ...
            msg = "%s: %sGot sig %s%s" % (
                msg,
                clr,
                SIGNALS_TO_NAMES_DICT.get(os.WTERMSIG(status), os.WTERMSIG(status)),
                color.reset,
            )
        ...
        msg = "%s: pid=%d; status=%d; %sunknown exit reason%s" % (
            msg,
            pid,
            status,
            color.red,
        ...
        for inst in self.instances:
            for dname, proc in list(inst.proc.items()):
                if daemon and dname != daemon:
                    continue
                self._test_proc_alive(
                    proc, expect, sig_ok, "%s on %s %s" % (dname, inst.name, how), sub
                )
    ...
    def _test_connections(self, sub, **kwargs):
        ...
        for inst in self.instances:
            ...
            sub.test(
                int(inst.info.peer_id),
                exp_peer_id,
                "%s should have peer-id %d" % (inst.name, exp_peer_id),
            )
            sub.test(
                int(inst.info.configured_peers),
                exp_num_peers,
                "%s should have %d peers" % (inst.name, exp_num_peers),
            )

            calc_con = (
                int(inst.info.active_peers)
                + int(inst.info.active_pollers)
                + int(inst.info.active_masters)
            )
            ...
            for i in inst.nodeinfo:
                if sub.test(
                    i.state,
                    "STATE_CONNECTED",
                    "%s -> %s connection" % (inst.name, i.name),
                ):
                    tot_con += 1
                if i.type == "peer":
                    sub.test(
                        i.peer_id,
                        i.self_assigned_peer_id,
                        "%s -> %s peer id agreement" % (inst.name, i.name),
                    )
            ...
            sub.test(
                tot_con,
                inst.num_nodes,
                "%s has %d/%d connected systems" % (inst.name, tot_con, inst.num_nodes),
            )
            sub.test(
                tot_con, calc_con, "%s should count connections properly" % inst.name
            )
    ...
    def test_oconfsplit(self, dir):
        ...
        for inst in self.instances:
            ...
            if not len(inst.group.master_groups):
                sub.test(
                    ret, False, "%s should not have a generated config" % inst.name
                )
                continue
    ...
    def test_nagios_restarts(self):
        ...
        for inst in self.instances:
            ...
            self.tap.test(
                lsize > inst.nslog_size, True, "%s must keep nagios.log" % inst.name
            )
    ...
    def _test_parents(self, sub, predicate):
        ...
        for inst in self.instances:
            ...
            sub.test(
                baseline,
                os.path.exists(inst.fpath("hnotify.log")),
                "%s should %ssend a host notification"
                % (inst.name, baseline == False and "not " or ""),
            )
        return sub.get_status() == 0
    ...
    def test_parents(self):
        ...
        for i in range(1, 4):
            ...
            hname = "master.%04d" % i
            offset = 20 - (i * 5)
            master.submit_raw_command(
                "SCHEDULE_HOST_CHECK;%s;%d" % (hname, now + offset)
            )

        master.submit_raw_command("START_EXECUTING_HOST_CHECKS")
        master.submit_raw_command("START_EXECUTING_SVC_CHECKS")
        time.sleep(30)
        status = self._test_until_or_fail(
            "parents",
            self._test_parents,
            90,
            predicate=lambda x: not x.name.startswith("pg1"),
        )
    ...
    def _test_active_checks(self, sub):
        ...
        for inst in self.instances:
            ...
            for i in inst.nodeinfo:
                ...
                sub.test(
                    i.assigned_hosts,
                    i.host_checks_executed,
                    "%s expected %s to run %s host checks, but thinks %s were run"
                    % (inst.name, i.name, i.assigned_hosts, i.host_checks_executed),
                )
                sub.test(
                    i.assigned_services,
                    i.service_checks_executed,
                    "%s expected %s to run %s service checks, but thinks %s were run"
                    % (
                        inst.name,
                        i.name,
                        i.assigned_services,
            ...
            sub.test(
                hchecks,
                inst.group.num_objects["host"],
                "%s should have all host checks accounted for" % inst.name,
            )
            sub.test(
                schecks,
                inst.group.num_objects["service"],
    ...
    def test_global_commands(self):
        ...
        if sub.done() != 0:
            return False
        return self._test_until_or_fail(
            "global command spreading", self._test_global_command_spread
        )
    ...
    def _test_notifications(self, sub, inst, **kwargs):
        ...
        sub.test(
            os.access(hpath, os.R_OK),
            hosts,
            "%s should %ssend host notifications"
            % (inst.name, ("", "not ")[hosts == False]),
        )
        sub.test(
            os.access(spath, os.R_OK),
            services,
            "%s should %ssend service notifications"
            % (inst.name, ("", "not ")[services == False]),
        )
        ...
        for inst in self.instances:
            ...
            for otype, query in list(queries.items()):
                ...
                ret = sub.test(
                    len(value),
                    expected[otype],
                    "%s should have %d %s, had %d"
                    % (inst.name, expected[otype], otype, len(value)),
                )
                if ret == False:
                    sub.diag("got:")
                    if "host" in otype:
    ...
    def test_passive_checks(self):
        ...
        for x in range(1, 4):
            for srv in self.masters.have_objects["service"]:
                ret = master.submit_raw_command(
                    "PROCESS_SERVICE_CHECK_RESULT;%s;2;Service plugin output" % (srv)
                )
                sub.test(ret, True, "Setting status of service %s" % srv)

        for host in self.masters.have_objects["host"]:
            ret = master.submit_raw_command(
                "PROCESS_HOST_CHECK_RESULT;%s;1;Plugin output for host %s"
                % (host, host)
            )
        ...
        if not self._test_until_or_fail(
            "passive check distribution", self._test_passive_checks, 30
        ):
            return sub.done() == 0
        ...
        for n in self.instances:
            ...
            self._test_until_or_fail(
                "passive check notifications for " + n.name,
                self._test_notifications,
                tap=sub,
                inst=n,
                hosts=should_have_notified,
                services=should_have_notified,
            )
        return sub.done() == 0

    def _test_ack_spread(self, sub):
        for inst in self.instances:
            value = inst.live.query(
                "GET hosts\nColumns: host_name\nFilter: acknowledged = 0"
            )
            ret = sub.test(
                0, len(value), "All host acks should register on %s" % inst.name
            )
            if not ret:
            ...
            ret = sub.test(
                0, len(value), "All service acks should register on %s" % inst.name
            )
            if not ret:
            ...
            sub.test(
                value,
                len(inst.group.have_objects["host"]),
                "Host acks should generate one comment each on %s" % inst.name,
            )
            value = inst.live.query(
                "GET comments\nStats: entry_type = 4\nStats: type = 2\nStatsAnd: 2"
            )[0][0]
            sub.test(
                value,
                len(inst.group.have_objects["service"]),
                "Service acks should generate one comment each on %s" % inst.name,
            )
    ...
    def test_acks(self):
        ...
        for inst in self.instances:
            ...
            if inst.name.startswith("pg1"):
                should_have_notified = False
            self._test_notifications(
                sub, inst, hosts=should_have_notified, services=should_have_notified
            )
        return sub.done() == 0
    ...
    def _schedule_downtime(self, sub, node, ignore={"host": {}, "service": {}}):
        ...
        for host in node.group.have_objects["host"]:
            ...
            sub.test(ret, True, "Scheduling downtime for %s on %s" % (host, node.name))
            ret = node.submit_raw_command(
                "SCHEDULE_HOST_DOWNTIME;%s;%d;%d;0;0;10;mon testsuite;Flexible downtime for host %s from %s"
                % (host, time.time() + 1, time.time() + 5, host, node.name)
            )
            sub.test(
                ret,
                True,
                "Adding flexible downtime for host %s from %s" % (host, node.name),
            )
        ...
        for srv in node.group.have_objects["service"]:
            ...
            sub.test(ret, True, "Scheduling downtime for %s on %s" % (srv, node.name))
            ret = node.submit_raw_command(
            ...
            sub.test(
                ret,
                True,
                "Adding flexible downtime for service %s on %s" % (srv, node.name),
            )
    ...
    def _test_downtime_count(self, sub, inst, hosts=0, services=0):
        ...
        sub.test(
            value,
            hosts,
            "%s: %d/%d hosts have scheduled_downtime_depth > 0"
            % (inst.name, value, hosts),
        )
        value = inst.live.query("GET downtimes\nStats: downtime_type = 2")[0][0]
        sub.test(
            value, hosts, "%s: %d/%d host downtime entries" % (inst.name, value, hosts)
        )
        value = inst.live.query(
            "GET comments\nColumns: host_name\nFilter: type = 1\nFilter: entry_type = 2\nAnd: 2"
        )
        sub.test(
            len(value),
            hosts,
            "%s: %d host downtime comments present, expected %d"
            % (inst.name, len(value), hosts),
        )
        if len(value) != hosts:
        ...
        sub.test(
            len(value),
            services,
            "%s: %d services have scheduled_downtime_depth > 0, expected %d"
            % (inst.name, len(value), services),
        )
        ...
        sub.test(
            value,
            services,
            "%s: %d/%d service downtime comments present"
            % (inst.name, value, services),
        )
    ...
    def _test_dt_count(self, sub, **kwargs):
        ...
        for inst in nodes:
            ...
            services = (arg_services, inst.group.num_objects["service"])[
                arg_services == None
            ]
    ...
    def test_downtime(self):
        ...
        for h in master.group.have_objects["host"]:
            master.submit_raw_command(
                "SCHEDULE_HOST_DOWNTIME;%s;%d;%d;1;0;%d;mon test suite;expire downtime test for %s"
                % (h, start_time, end_time, duration, h)
            )
        for s in master.group.have_objects["service"]:
            master.submit_raw_command(
                "SCHEDULE_SVC_DOWNTIME;%s;%d;%d;1;0;%d;mon test suite;expire downtime test for %s"
        ...
        self._test_until_or_fail(
            "downtime expiration", self._test_dt_count, 20, sub, hosts=0, services=0
        )
        sub.done()
        ...
        self._test_until_or_fail(
            "poller-scheduled downtime: spread", self._test_dt_count, 45, sub
        )

        host_downtimes = [
            x[0]
            for x in master.live.query(
                "GET downtimes\nColumns: id\nFilter: is_service = 0"
            )
        ]
        service_downtimes = [
        ...
        self._test_until_or_fail(
            "poller-scheduled downtime: deletion",
            self._test_dt_count,
            30,
            sub,
            hosts=0,
        ...
        master.submit_raw_command(
            "SCHEDULE_AND_PROPAGATE_TRIGGERED_HOST_DOWNTIME;%s;%d;%d;%d;%d;%d;%s;%s"
            % (
        ...
        parent_downtime = master.live.query(
            "GET downtimes\nColumns: id\nFilter: host_name = %s\nFilter: is_service = 0"
            % (poller.group_name + ".0001")
        )
    ...
    def _add_comments(self, sub, node):
        ...
        for host in node.group.have_objects["host"]:
            ...
            sub.test(ret, True, "Adding comment for %s from %s" % (host, node.name))
        for srv in node.group.have_objects["service"]:
            ...
            ret = node.submit_raw_command(
                ...
                % ("ADD_SVC_COMMENT", srv, _service_description, _host_name, node.name)
            )
            sub.test(
                ret,
                True,
                "Adding comment for service %s on %s from %s"
    ...
    def test_comments(self):
        ...
        if ret or self._test_until_or_fail(
            "comment spread", self._test_comment_spread, 30
        ):
            return False
        return True
    ...
    def _create_nodes(self):
        ...
        """
        port = 0
        self.masters = fake_peer_group(
            self.basepath, "master", self.num_masters, port, valgrind=self.valgrind
        )
        ...
        for p in self.opt_pgroups:
            ...
            group_name = "pg%d" % i
            fpg = fake_peer_group(
                self.basepath, group_name, p, port, valgrind=self.valgrind
            )
            fpg.mesh = self
    ...
    def create_playground(self, num_hosts=False, num_services_per_host=False):
        ...
        for pg in self.groups:
            sys.stdout.write(
                "Creating core config for peer-group %s with %d nodes\n  "
    ...
    def _destroy_database(self, inst, verbose=False):
        ...
        try:
            ...
            if verbose:
                print (
                    "Failed to drop db %s for instance %s: %s"
    ...
    def _create_database(self, inst):
        ...
        try:
            ...
            self.dbc.execute(
                "GRANT ALL ON %s.* TO merlin@'%%' IDENTIFIED BY 'merlin'" % inst.db_name
            )
            ...
            for row in self.dbc.fetchall():
                ...
                query = "CREATE TABLE %s.%s LIKE %s" % (inst.db_name, tname, tname)
...
class pasv_test_host(pasv_test):
    ...
    def query(self):
        return "%s WHERE host_name = '%s'" % (self.table, self.name)
...
def cmd_pasv(args):
    ...
    for arg in args:
        if arg.startswith("--nagios-cfg="):
            ...
            prettyprint_docstring(
                "pasv", cmd_pasv.__doc__, "Unknown argument: %s" % arg
            )
    ...
    for host in host_list:
        query = (
            ...
            "WHERE host_name = '%s' ORDER BY service_description ASC" % host.name
        )
    ...
    if num_hosts > host_list:
        print (
            "Can't run tests for %d hosts when only %d are configured"
            % (num_hosts, len(host_list))
        )

    if num_services > min_services:
        print (
            "Can't run tests for %d services / host when %s has only %d configured"
    ...
    while loops < num_loops:
        ...
        for t in test_objs:
            ...
            test(
                result,
                len(cmd),
                "%d of %d bytes written for %s" % (result, len(cmd), t.name),
            )
        ...
        for t in test_objs:
            ...
            row = dbc.fetchone()
            test(
                row[0] + delay > t.submit_time, True, "reasonable delay for %s" % t.name
            )
...
""" % (
    ...
    if os.getuid() != 0:
        expect_result = 0
        expect_out_oconf = (
            """rsync command: rsync -aotzc --delete -b --backup-dir=%s/backups /opt/monitor/etc -e ssh -C -o KbdInteractiveAuthentication=no localhost:/opt/monitor
"""
            % cache_dir
        )
    else:
