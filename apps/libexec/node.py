...
def cmd_status(args):
    ...
    for info in sinfo:
        ...
        name = "#%02d %d/%d:%d %s %s" % (
            iid,
            peer_id,
            int(info.get("active_peers", -1)),
            int(info.get("configured_peers", -1)),
            info["type"],
            info["name"],
        )

        if is_running:
            name += ": %sACTIVE%s - %s%.3fs%s latency" % (
                color.green,
                color.reset,
                lat_color,
                latency,
        ...
        if is_running and info["type"] == "peer" and sa_peer_id != peer_id:
            if conn_time + 30 > int(time.time()):
                print (
                    "%sPeer id negotiation in progress%s" % (color.green, color.reset)
                )
            else:
                print (
                    "%sPeer id mismatch: self-assigned=%d; real=%d%s"
                    % (color.yellow, sa_peer_id, peer_id, color.reset)
                )

        if iid and not node:
            print (
                "%sThis node is currently not in the configuration file%s"
        ...
        print (
            "Uptime: %s. Connected: %s. Last alive: %s%s%s"
        ...
        print (
            "Host checks (handled, expired, total)   : %d, %s%d%s, %d (%.2f%% : %.2f%%)"
            % (
                hchecks,
                hc_color,
                expired_hchecks,
        ...
        print (
            "Service checks (handled, expired, total): %d, %s%d%s, %d (%.2f%% : %.2f%%)"
            % (
                schecks,
                sc_color,
                expired_schecks,
    ...
    if len(oconf_bad):
        ...
        print (
            "%sPlease ensure that configuration synchronization works properly%s"
            % (color.yellow, color.reset)
        )
    ...
    for pg_id, d in pg_conf.items():
        ...
        for name, d2 in d.items():
            ...
            if have_first:
                if (
                    last_masters != cur_masters
                    or last_peers != cur_peers
                    or last_pollers != cur_pollers
                ):
                    pconf_bad[pg_id] = d
    ...
    if len(pconf_bad):
        ...
        for pg_id, bad_nodes in pconf_bad.items():
            ...
            print ("  peer-group %d: %s" % (pg_id, ", ".join(bad)))
        print (
            "%sPlease ensure that all peered nodes share neighbours%s"
            % (color.yellow, color.reset)
        )
    return exit_code
...
def cmd_ctrl(args):
    ...
    if not args:
        print ("Missing mandatory arguments to 'mon node ctrl'.")
        print ("Run 'mon node' for usage info on 'mon node ctrl'.")
        sys.exit(1)
