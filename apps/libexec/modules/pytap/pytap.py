...
class pytap(object):
    ...
    def __init__(self, name="Anonymous test-suite", master=False):
        ...
        self.verbose = 1
        # print buggy, todo, skip etc by default
        self.failures = []
        self.faildiag = []
    ...
    def _summarize(self):
        ...
        if self.parent:
            summary = self._colorize(
                "### @%d %s. " % (self.stop_time, self.suite), status
            )
        else:
        ...
        if self.tcount["ok"] and len(self.tcount) == 1:
            summary += self._colorize(
                "All self.num_tests tests passed in %.2fs" % self.runtime, "brightgreen"
            )
        else:
    ...
    def print_header(self, suite=False):
        ...
        self._indent()
        print(
            self._colorize("### @%d %s ###" % (self.start_time, suite), "description")
        )
    ...
    def ok(self, result, msg, flags=0):
        ...
        if self.planned_tests and self.num_tests > self.planned_tests:
            sys.stdout.write(
                "Test #%d out of self.planned_tests. What voodoo is this?!\n",
                self.num_tests,
            )
