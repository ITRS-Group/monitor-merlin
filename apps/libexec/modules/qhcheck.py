...
class QhClientError(QhError):
    pass


class QhServerError(QhError):
    pass
...
class QhCheck(object):
    ...
    def add_channel(self, channel):
        ...
        self.channels[channel.address] = multiprocessing.Process(
            name=channel.address, target=self.run_channel, args=(channel, their_end)
        )
...
class QhChannel(object):
    def __init__(
        self,
        address,
        path="/opt/monitor/var/rw/nagios.qh",
        options=None,
        subscribe=True,
    ):
        if not options:
            ...
            if not read:
                try:
                    if len(responses) > 0:
                        self.error(
                            responses[-1]
                        )  # could be that the last response was an error
                except QhError:
                    sys.stderr.write(
                        "%s: Error occured after query `%s'\n" % (self.address, query)
                    )
                    raise
                raise StopIteration
    ...
    def parse_responses(self, buf, initial_buffer=""):
        ...
        if self.options.linesep in (None, ""):
            responses.append(
                Response(initial_buffer + buf, self.options.pairsep, self.options.kvsep)
            )
            buf = ""
        else:
            while self.options.linesep in buf:
                ...
                if len(response_str) > 0:
                    responses.append(
                        Response(
                            initial_buffer + response_str,
                            self.options.pairsep,
                            self.options.kvsep,
                        )
                    )
                initial_buffer = ""
    ...
    def __init__(self, response_str, pairsep=None, kvsep=None):
        ...
        if not any([x in (None, "") for x in (pairsep, kvsep)]):
            self._response_dict = dict(
                [
                    y.split(kvsep, 1)
                    for y in response_str.strip().split(pairsep)
                    if len(y) > 0
                ]
            )
        else:
...
def run_tests():
    ...
    for response in qc_echo.query(echostr):
        ...
        assert (
            echostr == responses[0].value
        ), 'response "%s" doesn\'t equal original message "%s"' % (
            str(responses[0].value),
            echostr,
        )
...
if __name__ == "__main__":
    if len(sys.argv) > 1:
        ...
        with QhCheck() as qhcheck:
            ...
            if address[0] not in ("#", "@"):
                raise Exception(
                    "Prefix channel with # or @ for oneshot or subscription mode"
                )
