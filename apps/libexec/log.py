    ...
    if not "--list-files" in args:
        ...
        if mconf.num_nodes["poller"] or mconf.num_nodes["peer"]:
            ...
            retcode = subprocess.call(
                import_args, stdout=sys.stdout.fileno(), stderr=sys.stderr.fileno()
            )
            if retcode:
...
def cmd_purge(args):
    ...
    if os.path.exists("/opt/monitor/op5/pnp/perfdata/.trash"):
        subprocess.call(
            [
                "find",
                "/opt/monitor/op5/pnp/perfdata/.trash",
                "-mindepth",
                "1",
                "-delete",
            ]
    ...
    dbc.execute(
        "DELETE FROM report_data WHERE id NOT IN (SELECT id FROM (SELECT MAX(id) AS id FROM report_data WHERE timestamp < %s GROUP BY service_description, host_name) as tbl) AND timestamp < %s",
    ...
    for key in logformat:
        for log in glob.glob("%s/%s*" % (archive_dir, key)):
            try:
                if (
                    time.mktime(
                        time.strptime(log, "%s/%s" % (archive_dir, logformat[key]))
                    )
                    < oldest
                ):
                    os.remove(log)
