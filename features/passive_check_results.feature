...
Feature: Passive check result
    ...
    Scenario: As a master and peer I receive, from a peer, a passive check
       result for an object which I am responsible to check
       Given I start naemon with merlin nodes connected
        ...
        When the_peer sends event EXTERNAL_COMMAND
            | command_type   | 87                        |
            | command_string | PROCESS_HOST_CHECK_RESULT |
            | command_args   | hostB;1;Testoutput        |
        Then the_peer received event HOST_CHECK
            | name                | hostB      |
            | state.current_state | 1          |
            | state.state_type    | 1          |
            | state.plugin_output | Testoutput |
        And the_peer received event NOTIFICATION
            | host_name | hostB      |
            | state     | 1          |
            | output    | Testoutput |
        And the_poller should not receive HOST_CHECK
        And the_poller should not receive NOTIFICATION
        And the_peer should not receive EXTERNAL_COMMAND
        And the_poller should not receive EXTERNAL_COMMAND

    Scenario: Sending a passive check result to a local node for an object
        which is checked by a poller, make sure only the correct nodes receive
        the merlin packet
        Given I start naemon with merlin nodes connected
            | type   | name    | port | hostgroup   |
            | poller | poller1 | 4001 | pollergroup |
            | poller | poller2 | 4002 | emptygroup  |
        When I send naemon command PROCESS_SERVICE_CHECK_RESULT;hostA;PONG;1;Fromtest
        Then poller1 received event EXTERNAL_COMMAND
        And poller2 should not receive EXTERNAL_COMMAND
        And poller1 should not receive HOST_CHECK
        And poller2 should not receive HOST_CHECK
