		...
		mysql)
			...
			if test "$db_version"; then 
				while true; do
					...
					test -f "$f" || break
					eval "$mysql $db_name" < $f 2>&1 >>/tmp/merlin-sql-upgrade.log || abort "Incremental database upgrade from version $ver to version $nextver failed!"
					eval "$mysql $db_name -e \"UPDATE db_version SET version=$nextver\"" 2>&1 >> /tmp/merlin-sql-upgrade.log || abort "Failed to set new database version $nextver!"
					ver=$nextver
				done
			fi
...
install_apps ()
{
	...
	rm -f "$root_path/$bindir/mon"
	rm -f "$root_path/$bindir/op5"
	macro_subst "$src_dir/apps/mon.py" > "$root_path/$bindir/op5"
	ln -s op5 "$root_path/$bindir/mon"
	cp -a apps/libexec/* $root_path/$libexecdir/mon
	rm -f $root_path/$libexecdir/mon/-oconf
	cp oconf $root_path/$libexecdir/mon/-oconf
	chmod 755 $root_path/$bindir/op5
