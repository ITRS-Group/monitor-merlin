...
class nagios_object_importer
{
	...
	function get_object_index_table($obj_type)
	{
		...
		if(isset($this->idx_table[$obj_type])) {
			return($this->idx_table[$obj_type]);
		}
		...
		if(in_array($obj_type, $simple_objects) ||
		   in_array($obj_type, $template_objects)) {
			...
			foreach($srv as $obj_key => $obj) {
				$this->idx_table['service'][$obj_key] =
					$obj['host_name'] . ';' . $obj['service_description'];
			}
			return($this->idx_table['service']);
		}
	...
	function get_parents($host)
	{
		...
		if(!is_int($host)) {
			...
			$result = $this->sql_exec_query('SELECT id FROM host WHERE host_name = \'' .
