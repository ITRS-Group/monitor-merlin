...
{
	const host *key = *(const host **)a;
	const host *candidate = *(const host **)b;
...
{
	const service *key = *(const service **)a;
	const service *candidate = *(const service **)b;
	int result;
...
int main(int argc, char **argv)
{
	...
	t_set_colors(0);
	t_start("sorted list tests");
	sl = slist_init(ARRAY_SIZE(msg_list), compare);
	for (i = 0; i < ARRAY_SIZE(msg_list); i++) {
		slist_add(sl, msg_list[i]);
		host_list[i] = malloc(sizeof(host));
		service_list[i] = malloc(sizeof(service));
	...
	for (i = 0; i < ARRAY_SIZE(msg_list); i++) {
		char *k = slist_find(sl, msg_list[i]);
	...
	sl = slist_init(0, cmp_host);
	slist_set_list(sl, (void **)host_list, ARRAY_SIZE(msg_list), 0);
	for (i = 0; i < ARRAY_SIZE(msg_list); i++) {
		host *k;
		h_obj.name = msg_list[i];
		k = slist_find(sl, &h_obj);
		ok_str(k->name, msg_list[i], msg_list[i]);
	}
	free(sl);

	sl = slist_init(0, cmp_service);
	slist_set_list(sl, (void **)service_list, ARRAY_SIZE(msg_list), 0);
	for (i = 0; i < ARRAY_SIZE(msg_list); i++) {
		service *s;
		s_obj.host_name = s_obj.description = msg_list[i];
		s = slist_find(sl, &s_obj);
