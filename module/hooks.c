...
static int hook_service_result(merlin_event *pkt, void *data)
{
	...
	case NEBTYPE_SERVICECHECK_PROCESSED:
		...
		/* Did we end up here as a result of a network received event? */
		if (merlin_sender && recv_event) {
			if (recv_event->hdr.type == NEBCALLBACK_SERVICE_CHECK_DATA ||
				recv_event->hdr.type == NEBCALLBACK_SERVICE_STATUS_DATA) {
				/*
				 * Some other node sent us this check result which we simply
				 * registered locally. Block the packet from being propagated
				 * any further and make sure that the one who sent the check
				 * result is considered to be the one who ran the check
				 */
...
static int hook_host_result(merlin_event *pkt, void *data)
{
	...
	case NEBTYPE_HOSTCHECK_PROCESSED:
		...
		/* Did we end up here as a result of a network received event? */
		if (merlin_sender && recv_event) {
			if (recv_event->hdr.type == NEBCALLBACK_HOST_CHECK_DATA ||
